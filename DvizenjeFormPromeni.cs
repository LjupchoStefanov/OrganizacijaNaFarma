...
{
    public partial class DvizenjeFormPromeni : Form
    {
        ...
        private void tbIzdavanje_Validating(object sender, CancelEventArgs e)
        {
            if (tbIzdavanje.Text.Trim().Length == 0)
            {
                errorProvider1.SetError(tbIzdavanje, "Внеси број на издавање");
                e.Cancel = true;
            }
            else
            {
                string sh = tbIzdavanje.Text.Trim();
                foreach (char c in sh)
                {
                    if (!Char.IsDigit(c))
                    {
                        errorProvider1.SetError(tbIzdavanje, "Внеси цифри!");
                        e.Cancel = true;
                        return;
                    }
                }
                errorProvider1.SetError(tbIzdavanje, null);
                Izdavanje = int.Parse(tbIzdavanje.Text);
                e.Cancel = false;
            }
        }
    }
